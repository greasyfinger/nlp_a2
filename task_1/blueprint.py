...
class SequenceTaggingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return encoded["input_ids"].squeeze(), label_ids
...
def get_data(tokenizer):
    ...
    train_dataset = SequenceTaggingDataset(
        train_texts, train_labels, tokenizer, label_to_idx
    )
    val_dataset = SequenceTaggingDataset(val_texts, val_labels, tokenizer, label_to_idx)

    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)

    return train_loader, val_loader, idx_to_label
...
def run_epochs(model, tokenizer, run_name):
    ...
    train_loader, val_loader, idx_to_label = get_data(tokenizer)
